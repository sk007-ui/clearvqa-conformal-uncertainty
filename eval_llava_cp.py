...
def evaluate_aps(probs, labels, q_hat):
    ...
    for i in range(len(probs)):
        ...
        k = idx[0] + 1 if len(idx) > 0 else len(p)
        pred_set = sort_idx[:k]
        sizes.append(k)
...
def main():
    filepath = os.path.join("outputs_llava", "llava_clearvqa_final_logits.pkl")
    if not os.path.exists(filepath):
        print(f"Error: Could not find {filepath}")
        return

    data = load_data(filepath)
    print(f"Loaded {len(data)} samples from LLaVA ClearVQA.")

    labels       = np.array([OPTIONS.index(ex["answer"]) for ex in data])
    ...
    for split_name, probs in [("clear", clear_probs), ("ambig", ambig_probs)]:
        ...
        lac_size, lac_cov = evaluate_lac(test_probs, test_labels, lac_q)

        # APS
        aps_scores = compute_aps_scores(cal_probs, cal_labels)
        aps_q      = get_q_hat(aps_scores, alpha)
        aps_size, aps_cov = evaluate_aps(test_probs, test_labels, aps_q)

        res[split_name] = {
            "LAC": {"avg_set_size": lac_size, "coverage": lac_cov, "threshold": lac_q},
            "APS": {"avg_set_size": aps_size, "coverage": aps_cov, "threshold": aps_q},
        }

    print("\nResults:")
    print(json.dumps(res, indent=4))

    out_file = "cp_llava_clearvqa_results.json"
