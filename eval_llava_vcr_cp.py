...
def main():
    filepath = os.path.join("outputs_llava", "llava_vcr_final_logits.pkl")
    if not os.path.exists(filepath):
        print(f"Error: Could not find {filepath}")
        return

    data = load_data(filepath)
    print(f"Loaded {len(data)} samples from LLaVA VCR.")

    # VCR format already uses integer answer_idx
    ...
    out_file = "cp_llava_vcr_results.json"
