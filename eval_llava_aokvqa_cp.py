...
def main():
    filepath = os.path.join("outputs_llava", "llava_aokvqa_final_logits.pkl")
    if not os.path.exists(filepath):
        print(f"Error: Could not find {filepath}")
        return

    data = load_data(filepath)
    print(f"Loaded {len(data)} samples from LLaVA A-OKVQA.")

    # Format includes answer_idx directly
    ...
    out_file = "cp_llava_aokvqa_results.json"
